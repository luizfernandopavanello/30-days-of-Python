...
```sh
treviit@Treviit:~$ pip install pip
```
...
```sh
treviit@Treviit:~$ pip install pandas
```

```py
treviit@Treviit:~$ python
Python 3.9.6 (default, Apr 12 2022, 14:53:21)
[Clang 11.0.0 (clang-1100.0.33.8)] on darwin
Type "help", "copyright", "credits" or "license" for more information.
>>> import pandas
```
...
```sh
treviit@Treviit:~$ pip show pandas
Name: pandas
Version: 1.2.3
...
```sh
treviit@Treviit:~$ pip show --verbose pandas
Name: pandas
Version: 1.2.3
Summary: Powerful data structures for data analysis, time series, and statistics
Home-page: http://pandas.pydata.org
...
```py
treviit@Treviit:~$ pip install requests
```
...
```sh
treviit@Treviit:~/Desktop/30DaysOfPython$ python
...
>>> greet.greet_person('Case', 'Sprawl')
'Case Sprawl, welcome to 30DaysOfPython Challenge!'
>>>
```
